...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // 🔑 الانضمام لغرفة الشركة الخاصة (الحل السحري للإشعارات)
    if (user.companyId) {
      socket.emit("join_company", user.companyId);
    }

    socket.on("new_notification", (data) => {
      // إذا كان المدير والرسالة للمدير، أو الرسالة موجهة للمستخدم شخصياً
      if (
  ...
  if (user?.role === "employee")
    mobileMenu.splice(1, 0, { path: "/dashboard", icon: "📍", label: "تسجيل" });

  return (
    <div
      className="flex h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 font-sans overflow-hidden"
      dir="rtl"
    >
      <ToastContainer />
      ...
      <div className="flex-1 flex flex-col h-full relative">
        <header className="lg:hidden bg-white dark:bg-slate-800 shadow-sm border-b dark:border-slate-700 p-4 flex justify-between items-center z-30 shrink-0">
          ...
          <button
            ...
            className="p-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-white hover:bg-slate-200 transition"
          >
            <svg
