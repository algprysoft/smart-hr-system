...
const QrStation = () => {
  ...
  return (
    <div
      ...
      dir="rtl"
    >
      <h1 className="text-4xl md:text-5xl font-bold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500 text-center">
        Smart HR 🚀
      </h1>
      <p className="text-gray-400 mb-10 text-lg md:text-xl text-center">
        امسح الكود لتسجيل الحضور فوراً
      </p>

      <GlassCard className="relative p-6 md:p-8 bg-white/10 backdrop-blur-lg border-white/20 !rounded-3xl !shadow-2xl flex justify-center">
        <div className="bg-white p-4 rounded-2xl w-full max-w-[300px]">
          <QRCodeCanvas
            value={qrValue}
            size={280}
      ...
      <div className="mt-12 text-center w-full px-4">
        <p className="text-sm md:text-lg font-mono text-yellow-400 mb-6 bg-yellow-400/10 px-4 py-2 rounded-full inline-block border border-yellow-400/20">
          ⚠️ يتجدد الكود تلقائياً كل {refreshRate / 1000} ثواني
        </p>
        <br />
        <button
          onClick={() => navigate("/dashboard")}
          className="mt-4 px-8 py-3 border border-gray-600 rounded-full text-gray-400 hover:text-white hover:border-white hover:bg-white/5 transition font-bold w-full md:w-auto"
        >
          العودة للوحة التحكم
        </button>
      </div>
