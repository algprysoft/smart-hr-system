...
import { API_URL } from "../config"; // ✅ استخدام الرابط الموحد
...
const Finance = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 1. جلب السلف (للكل)
        const advRes = await axios.get(`${API_URL}/finance/advances`, {
          headers,
        });
        setAdvances(advRes.data);

        // 2. إذا كان مديراً، اجلب المكافآت والموظفين
        if (user.role === "admin") {
          const bonRes = await axios.get(`${API_URL}/finance/bonuses`, {
            headers,
          });
  ...
  const handleRequestAdvance = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API_URL}/finance/advance`, advanceForm, {
  ...
  const handleDeleteAdvance = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/finance/advance/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setRefresh((p) => p + 1);
    } catch (err) {
      alert("فشل الحذف: " + (err.response?.data?.message || ""));
    }
  };
  ...
  const handleAddBonus = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API_URL}/finance/bonus`, bonusForm, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const inputStyle =
    "p-3 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-slate-700 text-gray-800 dark:text-white outline-none transition-all w-full focus:ring-2 focus:ring-blue-500";
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-700 dark:text-white px-4 py-2 rounded-xl font-bold transition"
        >
          <span>الرئيسية</span> 🏠
        </button>
      </div>
        ...
        <div className="space-y-8">
          <GlassCard className="border-l-4 border-blue-500">
            ...
            >
              ...
              <button
                type="submit"
                className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-bold shadow transition"
              >
                إرسال
              </button>
            </form>
          ...
          <ElegantTable>
            ...
            <tbody>
              {advances.map((adv) => (
                <TableRow key={adv.id}>
                  <TableCell className="font-bold text-gray-800 dark:text-white">
                    {adv.User?.name || "غير معروف"}
                  </TableCell>
                    ...
                    <div className="flex gap-2 items-center">
                      {user.role === "admin" && adv.status === "pending" && (
                        <>
                          <button
                            onClick={() =>
                              handleUpdateStatus(adv.id, "approved")
                            }
                            className="text-green-600 bg-green-100 p-2 rounded hover:bg-green-200"
                          >
                            ✓
                          </button>
                          <button
                            onClick={() =>
                              handleUpdateStatus(adv.id, "rejected")
                            }
                            className="text-red-600 bg-red-100 p-2 rounded hover:bg-red-200"
                          >
                            ✕
                          </button>
                        </>
                      ...
                      {adv.status === "pending" &&
                        (user.role === "admin" || user.id === adv.userId) && (
                          <button
            ...
            >
              <div className="w-full md:w-64">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setBonusForm({ ...bonusForm, userId: e.target.value })
                  }
                >
              ...
              <button
                type="submit"
                className="w-full md:w-auto bg-purple-600 text-white px-8 py-3 rounded-lg font-bold shadow transition"
              >
                إضافة
              </button>
            </form>
