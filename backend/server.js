...
// ✅ تعريف العلاقات (النسخة المعتمدة)
// الشركة تملك كل شي
...
io.on('connection', (socket) => {
    ...
    socket.on('send_location', (data) => {
        if(data.companyId) io.to(`company_${data.companyId}`).emit('update_map', data);
    });
});
...
try { require('./src/utils/cronJobs')(); } catch (e) {}

// تحديث قاعدة البيانات (Alter)
sequelize.sync({ alter: true }).then(() => {
    console.log('✅ System Database Synced & Ready');
